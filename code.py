...
def categorical(df):
    ...
    categorical_var=cat_var.columns.tolist()
...
def numerical(df):
    ...
    """
    cat_var=df.select_dtypes(include='number')
    numerical_var=cat_var.columns.tolist()
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    agg_dict1 = {'mean':np.mean,'max':np.max,'min':np.min,'sum':np.sum,'len':len}
    aggregated_value=pd.pivot_table(df,index=df[date_col].dt.month,values=[agg_col],aggfunc= {agg_col:agg_dict1[agg]})
...
def group_values(df,col1,agg1):
   ...
   Keyword arguments:
    ...
    """
    agg_dict = {'mean':np.mean,'max':np.max,'min':np.min,'sum':np.sum,'len':len}
    grouping = df.groupby(col1).agg(agg_dict[agg1])
    return grouping
...
def convert(df,celsius):
    ...
    """
    df[celsius]=df[celsius].apply(lambda x: ((x*1.8)+32))
    converted_temp=df
...
agg_values_ina_month=agg_values_ina_month(weather,'Date/Time','Temp (C)','mean')
